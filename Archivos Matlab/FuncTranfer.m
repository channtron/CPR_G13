...
%  Tau=[Im1*R1*K(1,1);Im2*R2*K(2,2);Im3*R3*K(3,3)];
  G1s=tf(Tau(1),[3.4915  5.4365e-6 0]);
  G2s=tf(Tau(2),[6.9901  8.6930e-6 0]);
  G3s=tf(Tau(3),[1.0899  6.0046e-5 0]);


  G1s=tf(Tau(1)/5.4365e-6,[3.4915 5.4365e-6 0]/5.4365e-6);
  G2s=tf(Tau(2)/8.6930e-6,[6.9901  8.6930e-6 0]/8.6930e-6);
